...
public class MainActivity extends Activity implements OnLongClickListener {
	...
	private OnClickListener onClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			if (v == btn_main_call) {// 点击打电话
				...
				// 1). 创建一个Intent(隐式)
				// 08-28 03:27:09.976: I/ActivityManager(1222): Displayed
				// com.android.dialer/.DialtactsActivity: +535ms
				String action = "android.intent.action.DIAL";
				action = Intent.ACTION_DIAL;// 一样的
				Intent intent = new Intent(action);
				// 2). 携带数据
				String number = et_main_number.getText().toString();
				intent.setData(Uri.parse("tel:" + number)); // <data
															// android:scheme="tel"
															// />
				...
				// 1). 创建一个Intent(隐式)
				// 08-28 04:00:02.420: I/ActivityManager(1222): Displayed
				// com.android.mms/.ui.ComposeMessageActivity: +132ms
	...
	public boolean onLongClick(View v) {
		if (v == btn_main_call) {// 长按打电话
			...
			// 1). 创建一个Intent(隐式)
			// 08-28 03:38:59.717: I/ActivityManager(1222): Displayed
			// com.android.phone/.PrivilegedOutgoingCallBroadcaster: +570ms
